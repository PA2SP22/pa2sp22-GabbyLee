 ...
 * Sources:
        https://www.techiedelight.com/string-representation-integer-array-cpp/
 */
...
// A string containing the contents of an array.
string PrepareForDisplay(int values[], int size, char separator) {
int i;
stringstream ss;
...
// Swap the positions of two values in an integer array.
bool SwapValues(int values[], int size, int index1, int index2) {
  int temp1;
  int tempTwo;
  bool isSwapped;
...
temp1 = values[index1];
tempTwo = values[index2];
values[index1] = values[index2];
values[index2] = temp1;
}
  if ((values[index1] == tempTwo) && (values[index2] == temp1)) {
  isSwapped = true;
} else {
    isSwapped = false;
}
